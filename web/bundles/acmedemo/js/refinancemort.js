function getEquity()
{
    ...
    jQuery("#max-loan").val(Globalize.format(loanval, "n", "en-US"));
    jQuery("#equity").val(Globalize.format(Math.abs(equity), "n", "en-US"));

    if (equity <= 0)
    {
        jQuery("#equityamount").unbind("change");
        jQuery("#equityamount").val('0');
        jQuery("#new-mort").val(parseInt(jQuery("#leftamount").val().replace(/[^\d\.]/g,
                    "")));
        t = jQuery("#equityamountslider").slider({
            range: "min",
            value: parseInt(jQuery("#equityamount").val().replace(/[^\d\.]/g,
                    "")),
            min: 0,
            max: 0,
            step: 0,
        ...
        jQuery("#msg").html('Equity Not Avilable');
    }
    else
    {
       jQuery("#new-mort").val(Globalize.format((parseInt(jQuery("#equityamount").val().replace(/[^\d\.]/g,
                    ""))+parseInt(jQuery("#leftamount").val().replace(/[^\d\.]/g,
                    ""))), "n", "en-US"));
        t = jQuery("#equityamountslider").slider({
            range: "min",
            value: parseInt(jQuery("#equityamount").val().replace(/[^\d\.]/g,
            ...
            slide: function (a, b) {
                jQuery("#equityamount").val(Globalize.format(Math.round(b.value), "n", "en-US"));
                jQuery("#new-mort").val(Globalize.format((parseInt(jQuery("#equityamount").val().replace(/[^\d\.]/g,
                    ""))+parseInt(jQuery("#leftamount").val().replace(/[^\d\.]/g,
                    ""))), "n", "en-US"));
                debt_console();
        ...
        jQuery("#msg").html('Available Equity = <span style="color:#993A00;">' + Globalize.format(Math.abs(equity), "n", "en-US") + '</span>');
    }
}
...
function getPenalty()
{
    ...
    if (jQuery(".mort-type:checked").val() == 'variable')
    {
        ...
        jQuery("#mortgage-penalty").val(Globalize.format(Math.abs(total_penalty), "n", "en-US"));
    }
    else
    {
...
jQuery(document).ready(function () {
    ...
    jQuery("#loanamount").keyup(function () {
        jQuery("#loanamount").val(Globalize.format(jQuery("#loanamount").val().replace(/[^\d\.]/g, "")), "n", "en-US");
        getPenalty();
        getEquity();
    });

    jQuery("#leftamount").keyup(function () {
        jQuery("#leftamount").val(Globalize.format(jQuery("#leftamount").val().replace(/[^\d\.]/g, "")), "n", "en-US");
        getEquity();
         getPenalty();
    });
    ...
    jQuery("#loaninterest").keyup(function () {
        jQuery("#loaninterest").val(Globalize.format(jQuery("#loaninterest").val().replace(/[^\d\.]/g, "")), "n", "en-US");
        getPenalty();
    });

    jQuery("#newloaninterest").keyup(function () {
        jQuery("#newloaninterest").val(Globalize.format(jQuery("#newloaninterest").val().replace(/[^\d\.]/g, "")), "n", "en-US");
        getPenalty();
    });
    
    jQuery(".debt-outstand").keyup(function () {
        jQuery(this).val(Globalize.format(jQuery(this).val().replace(/[^\d\.]/g, "")), "n", "en-US");
        debt_console();
    });
    ...
    H("Property Value", 1E6, 5E3, 50E4, 20, 20, 30, 0.5, 25, 2.99, 1.99);
    getPenalty();
    getEquity();
    debt_console();
});
...
function H(a, e, z, b, k, I, A, M, K, R, S) {
    ...
    jQuery("#loanamount").val(Globalize.format(b, "n", "en-US"));
    jQuery("#leftamount").val(Globalize.format(25E4, "n", "en-US"));
    ...
    s = jQuery("#leftamountslider").slider({
        ...
        step: z,
        slide: function (a, b) {
            jQuery("#leftamount").val(Globalize.format(Math.round(b.value), "n", "en-US"));
            getEquity();
            debt_console();
        },
        change: function (a) {
...
function debt_console()
{
    ...
    jQuery(".debt-outstand").each(function(){
        total_debt = parseInt(total_debt)+parseInt($(this).val().replace(/[^\d\.]/g,
                ""));
    });
    jQuery("#non-mort-debt").val('$'+Globalize.format(total_debt, "n", "en-US"));
    jQuery("#new-non-mort-debt").val('$'+Globalize.format(total_debt, "n", "en-US"));
    jQuery("#mort-debt").val('$'+jQuery("#leftamount").val());
    jQuery("#new-mort-debt").val('$'+jQuery("#new-mort").val());
    jQuery("#total-debt").val('$'+Globalize.format((parseInt($("#leftamount").val().replace(/[^\d\.]/g,
                ""))), "n", "en-US"));
    jQuery("#new-total-debt").val('$'+Globalize.format((parseInt(jQuery("#mortgage-penalty").val().replace(/[^\d\.]/g,
                ""))+parseInt(total_debt)+parseInt($("#new-mort").val().replace(/[^\d\.]/g,""))), "n", "en-US"));
    jQuery("#exist-rate").val(Globalize.format(jQuery("#loaninterest").val().replace(/[^\d\.]/g,
                ""), "n", "en-US")+" %");
     jQuery("#new-rate").val(Globalize.format(jQuery("#newloaninterest").val().replace(/[^\d\.]/g,
                ""), "n", "en-US")+" %");
    emi_calculate();
    
    var num = parseInt(jQuery("#loanamount").val().replace(/[^\d\.]/g, ""));
    console.log(num * 0.8);
    console.log(parseInt($("#leftamount").val().replace(/[^\d\.]/g,"")));
    if((num * 0.8) > (parseInt($("#new-total-debt").val().replace(/[^\d\.]/g,""))))
    {
...
{
    var principle = jQuery("#new-mort").val().replace(/[^\d\.]/g, "");
    var num_years = 25;
    var rate = jQuery("#newloaninterest").val().replace(/[^\d\.]/g,"");
    ...
    var installment = principle * ((desired_interest_rate * Math.pow((1 + desired_interest_rate), number_of_payments)) / (Math.pow((1 + desired_interest_rate), number_of_payments) - 1));
     ...
     jQuery("#new-pay").val('$' + Globalize.format(Math.round(installment), "n", "en-US"));
   
    var old_principle = jQuery("#mort-debt").val().replace(/[^\d\.]/g, "");
    var old_rate = jQuery("#loaninterest").val().replace(/[^\d\.]/g,"");
    var old_num_years = 25;
    var old_payment_frequency = jQuery("#month").find('option:selected').val();

    
    var old_number_of_payments = old_num_years * get_pay_periods(old_payment_frequency);

    var old_desired_interest_rate = (old_rate / 100) / get_pay_periods(old_payment_frequency);

    var old_installment = old_principle * ((old_desired_interest_rate * Math.pow((1 + old_desired_interest_rate), old_number_of_payments)) / (Math.pow((1 + old_desired_interest_rate), old_number_of_payments) - 1));

   
    jQuery("#old-pay").val('$'+Globalize.format(Math.round(old_installment), "n", "en-US"));
